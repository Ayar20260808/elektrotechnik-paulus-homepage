...
es hoehenbegrenzt, dort schneidet der Browser links und rechts ab, und erst
dort wirkt die Lage. Gemessen am 19.09.2026 bei 390 px Fensterbreite, welcher
Ausschnitt der 1600 px breiten `hero-tiefenerder.jpg` ankommt:

    50 % (Voreinstellung)   x  548 .. 1053
    12 %                    x  178 ..  682
    19 %                    x  246 ..  751
    25 %                    x  304 ..  809
...
| — | ~~**`leistungvde.html` vom Server loeschen**~~ | **Erledigt 12.09.2026, von Irfan gemeldet.** Nicht unabhaengig nachgeprueft -- diese Sitzung kommt nicht an den Server. Urspruenglich: ohne Bindestrich, 63,68 KiB, 9 Tage alt -- der Rest des Namensfehlers vom 03.09. Sie steht **im Webverzeichnis und ist oeffentlich abrufbar** unter `/leistungvde.html`, als veraltete Zweitfassung der VDE-Seite. `robots.txt` sperrt nichts (`Allow: /`). Genau der doppelte Inhalt, der unter Punkt 4 als Risiko benannt ist -- nur nicht bei Wix, sondern auf dem eigenen Server. Im Projekt gibt es die Datei nicht, sie kann also ersatzlos weg |
